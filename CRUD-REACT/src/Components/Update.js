...
function Update({ onClose, onUpdateUser }) {
    ...
    const handleSubmit = () => {
        if (!id || !name || !course || !gender || !rollNo) {
            alert('Please enter all fields');
            return;
        }

        onUpdateUser(id, { name, course, gender, rollNo });
        onClose();
    };
    ...
    return (
        <div className="fixed top-0 h-screen w-screen left-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
            <div className="bg-white rounded-lg max-w-xl p-8">
                <h1 className="text-2xl font-semibold text-center text-red-700 mb-6">Update Account</h1>
                ...
                <div className="flex justify-center mt-6">
                    <button onClick={handleSubmit} className="text-white bg-green-700 hover:bg-green-800 font-medium rounded-full py-2 px-6 mr-4">Update</button>
